...
local function GSOE()
    ...
    if SERVER and ent_fire then
        ...
        ent_fire.Think = function(self)
            ...
            dmg:SetDamage(math.floor(10 * fireMult:GetFloat() * math.min(1, (CurTime() - self.SpawnTime) / 3 * 0.5 + 0.5)))
    ...
    if SERVER and ent_thermite then
        ent_thermite.Think = function(self)
            ...
            if self:GetArmed() then
                ...
                local dmg = DamageInfo()
                dmg:SetDamageType(DMG_BURN)
                dmg:SetDamage(math.floor(75 * 0.4 * thermMult:GetFloat() * math.min(1, (CurTime() - self.ArmTime) / 1 * 0.5 + 0.5)))
                dmg:SetDamageForce(Vector(0, 0, 0))
                dmg:SetDamagePosition(self:GetPos())
                dmg:SetInflictor(self)
